...
class Model(torch.nn.Module):
    def __init__(self, num_actions):
        ...
        self.pi = torch.nn.Sequential(
            torch.nn.Linear(in_features=4,out_features=4),
            torch.nn.ReLU(),
            torch.nn.Linear(in_features=4,out_features=num_actions),
        )
    def forward(self, x):
        v = self.v(x)
        pi = self.pi(x)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max-steps', type=int, default=10_000_000//8//512, help='Number of training steps to run. One step is one weight update.')
    parser.add_argument('--lr', type=float, default=1e-2, help='Learning rate.')
    parser.add_argument('--rollout-length', type=int, default=512, help='Length of rollout.')
    ...
    parser.add_argument('--entropy-loss-coeff', type=float, default=0.001, help='Coefficient for the entropy loss.')
    parser.add_argument('--target-kl', type=float, default=0.01, help='Target KL divergence.')
    parser.add_argument('--minibatch-size', type=int, default=128, help='Minibatch size.')
    parser.add_argument('--num-minibatches', type=int, default=16, help='Number of minibatches.')
